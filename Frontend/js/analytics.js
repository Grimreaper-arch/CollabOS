...
if (isTeacherAnalytics) {
    ...
    const users = get("users"); const studentEmails = [...new Set(scope.map((task) => task.assigneeEmail))];
    document.getElementById("breakdown").innerHTML = studentEmails.length ? studentEmails.map((email) => { const studentTasks = scope.filter((task) => task.assigneeEmail === email); const done = studentTasks.filter((task) => task.status === "completed").length; const student = users.find((user) => user.email === email); const percent = rate(done, studentTasks.length); return `<div><div class="flex justify-between gap-4 text-sm"><span>${(student?.fullName || email)}</span><span class="text-[#bcbcc6]">${done}/${studentTasks.length} complete</span></div><div class="mt-2 h-2 overflow-hidden rounded bg-[#15161a]"><div class="h-full bg-[#54d7ff]" style="width:${percent}%"></div></div><p class="mt-1 text-right text-xs text-[#bcbcc6]">${percent}%</p></div>`; }).join("") : '<p class="text-sm text-[#bcbcc6]">Assign tasks to students to see their progress here.</p>';
} else {
